...
export function getThaliStats(thalis) {
  ...
  const allThalisPriceLIst = thalis.map(thali => thali.price);
  const cheapest = Math.min(...allThalisPriceLIst);
  const costliest = Math.max(...allThalisPriceLIst);
  const totalThalisPrice = thalis.reduce((totalPrice, thali) => totalPrice + thali.price, 0);
  const avgPrice = (totalThalisPrice / totalThalis).toFixed(2);
...
export function searchThaliMenu(thalis, query) {
  ...
  return thalis.filter((thali) => {
    const nameMatch = thali.name.toLowerCase().includes(queryInLowerCase);
    const itemMatch = thali.items.some((item) =>
      item.toLowerCase().includes(queryInLowerCase)
    );
...
export function generateThaliReceipt(customerName, thalis) {
  ...
  return `${customerName.toUpperCase()}  - Items: `
}
